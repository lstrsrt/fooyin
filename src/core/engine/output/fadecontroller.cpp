...
uint64_t FadeController::armActiveFade(uint64_t transitionId, bool fadeOut)
{
    if(transitionId == 0) {
        return 0;
    }
                                            ...
                                            double /*volume*/, uint64_t transitionId)
{
    ...
    const auto& manualSpec = crossfadingValues.manualChange;
    if(!manualSpec.isConfigured()) {
        return false;
    }

    if(isPlaying && manualSpec.effectiveOutMs() > 0 && m_state != FadeState::FadingToManualChange) {
        m_pendingManualTrack        = track;
        m_pendingManualFadeInMs     = manualSpec.effectiveInMs();
    ...
    if(manualSpec.effectiveInMs() > 0) {
        m_manualFadeInPendingMs  = manualSpec.effectiveInMs();
        m_manualFadeTransitionId = transitionId;
    }
