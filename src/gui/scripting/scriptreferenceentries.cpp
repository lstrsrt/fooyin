...
const std::vector<ScriptReferenceEntry>& scriptReferenceEntries()
{
    ...
    static const std::vector Entries = {
        ...
        functionEntry("urlencode", u"$urlencode(text)"_s, tr("Percent-encodes text for use in URLs")),
        functionEntry("timems", u"$timems(milliseconds)"_s, tr("Formats milliseconds as time")),
        functionEntry("get", u"$get(name)"_s, tr("Returns the value stored in a script variable")),
